...
#violin_per_sample(data_mat = data_mat, x = kanal, y = "dataset", line = NA)
...
The percentage of each file that are of a given level can be calucated from the result. Here you find the percent that are higher than the found gate.
...
* you can try to use the variable minimum in find_gaussian_gates_second_top(). That will force the dataset to either use this value as the minimum or find the first minimum after this value as the minimum.

* you might also just define your own gate. By using the same value fro all files. 
...
Unfortunately it looks like the gate is placed in the middel of the top. This is "just" a smoothing and visualisation problem. If we count how many cells that are above the upper gate found here we get..
...
this means that the area between the two gates consist of
...
```{r}
find_split_first_second_top(data = data, channel = kanal)
```
