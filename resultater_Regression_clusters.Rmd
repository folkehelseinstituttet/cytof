...
prepare_data <- function(){
    klu <- read.csv2(fs::path(params$utSti, paste0("antallPerClusterOgPat_k_", ant_klu_i , "_seed", params$seed, params$selcected_name,  ".csv")))
  
  rownames(klu) <- klu$X
   
  klu <- klu[,-(which(colnames(klu) == "X"))]
 
  antallPerKluster <- apply(klu, 2, sum)
  antallPerFil <- apply(klu, 1, sum)
 # summary(antallPerFil)
  ...
  klu <- (klu + 1)/matrix(rep(antallPerFil + ant_klu_i, nrow(klu)), ncol = length(antallPerFil), byrow = T)
...
d$StatusTidSex <- factor(d$StatusTidSex, levels = c("Severe, T1 F", "Severe, T1 M", "Moderat, T1 F", "Moderat, T1 M",  "Severe, T2 M", "Moderat, T2 F", "Moderat, T2 M", "Control F",  "Control M"  ))
...
pairedPlot <- function(i){ 
  plot( rep(1, nrow(ddT1)), ddT1[,i], xlim = c(0.5,3.5), axes = F, cex = 2,  main = paste("kluster", i, ", antall i kluster = ", antallPerKluster[i]), xlab = "Time", col = ddT1$col, ylim = c(min(min(dK[,i]), min(dd[,i])), c(max(max(dK[,i]), max(dd[,i])))), pch = 16 )
...
violinPlot <- function(dat, i){
  ...
  g <- ggplot(d, aes(x = x, y = y, group = group)) + 
...
```{r, echo = F, comment= FALSE, message= FALSE, warning= FALSE, fig.width= 12, fig.height=8}
   ...
   if(res$coefficients$mean["Alder", "Pr(>|z|)" ] < 0.05 ){
     ...
     if(res$coefficients$mean["SexM", "Pr(>|z|)" ] < 0.05){
        ...
        if(any(res$coefficients$mean[2:9, "Pr(>|z|)"] < 0.05)){
           ...
           g <- ggplot(d, aes(x = StatusTidSex, y = d[,i])) +
    ...
    g <- g + theme(axis.text = element_text(size = 20), axis.title = element_text(size = 20), title  = element_text(size = 20)) + 
    ylab("proportion") 
    g <- g + ggtitle(paste0("kluster ", i, ": antall i kluster = ", antallPerKluster[i]))
    print(g)
 }
}
